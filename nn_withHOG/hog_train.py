...
def train_over(list1, layer1, layer2, loss):
    loss_sum = 0
    for i in range(45000):
        ...
        if (i + 1) % 60 == 0:
            layer1.update(0.0019, 0.009)
            layer2.update(0.009, 0.08)
            if (i + 1) % 3000 == 0:
...
def main():
    train_list = []
    read_and_deal(train_list)
    w1 = np.random.normal(loc=0.0, scale=0.022, size=140*2187)
    w1 = np.reshape(w1, (140, 2187))
    w2 = np.random.normal(loc=0.0, scale=0.083, size=10*140)
    w2 = np.reshape(w2, (10, 140))
    b1 = np.zeros((140, 1), dtype=np.float64)
